...
#Bateas
B = {c : range(sub_c*(c+3),sub_c*(c+4)) for c in C}
...
for n in C:
    ...
    for i in range(sub_c):
        P_c[G1_list[i]] = S_list[i]     # G1 depende del subcaserón
        P_c[G2_list[i]] = G1_list[i]    # G2 depende de G1
        P_c[B_list[i]]  = G2_list[i]    # Batea depende de G2

    # Subcaserones no tienen predecesor
    for s in S_list:
        P_c[s] = None
...
range(1,time_l)
#======= PARAMETROS ======
Mass_drill = {c: 3 for n in C for c in S[n]}
Mass_blast = {c: 3 for n in C for c in S[n]}
Mass_extr  = {c: 3 for n in C for c in S[n]}
...
for act, pre in P_c.items():
    if pre is None:
        continue
    for t in range(1, time_l):
        m.addConstr(
            gp.quicksum(x_hat[i, act, t, 0] for i in I_h if (i,act,t,0) in x_hat)
            <= I_h_v * z_bar[pre, t-1]
        )
        m.addConstr(
            gp.quicksum(y_hat[j, act, t] for j in J)
            <= J_val * z_bar[pre, t-1]
        )
        m.addConstr(
            gp.quicksum(z_hat[k, act, t] for k in K)
            <= K_val * z_bar[pre, t-1]
        )
...
for n in C:
    for idx, c_sub in enumerate(S[n]):
        g1 = list(G_1[n])[idx]
        g2 = list(G_2[n])[idx]
        b  = SB_of_sub[c_sub]   # tu mapping

        for t in range(1, time_l):
            # Para volar subcaserón: debe estar lista la galería 2 y la batea antes
            m.addConstr(gp.quicksum(y_hat[j, c_sub, t] for j in J) <= J_val * z_bar[g2, t-1])
            m.addConstr(gp.quicksum(y_hat[j, c_sub, t] for j in J) <= J_val * z_bar[b,  t-1])

            # Para extraer subcaserón: también debe estar listo g2 y batea antes
            m.addConstr(gp.quicksum(z_hat[k, c_sub, t] for k in K) <= K_val * z_bar[g2, t-1])
            m.addConstr(gp.quicksum(z_hat[k, c_sub, t] for k in K) <= K_val * z_bar[b,  t-1])
